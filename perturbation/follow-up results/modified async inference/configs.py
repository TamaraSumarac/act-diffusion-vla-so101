...
class RobotClientConfig:
    ...
    duration: float | None = None   # seconds; None = run until Ctrl+C
    display_data: bool = False      # stream camera frames to Rerun

    # ADDED (2026/09/02): client-side obs pairing (n_obs_steps>1 policies, i.e.
    # diffusion) + optional JPEG transport for observation images
    pair_observations: bool = False
    jpeg_quality: int = 0  # 0 = raw transport; 85-95 typical when enabled
    ...
    def __post_init__(self):
        ...
        if not (0 <= self.jpeg_quality <= 100):
            raise ValueError(f"jpeg_quality must be in [0, 100] (0 = raw transport), got {self.jpeg_quality}")
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            # ADDED (2026/08/31 fields, included 2026/09/02):
            "duration": self.duration,
            "display_data": self.display_data,
            # ADDED (2026/09/02):
            "pair_observations": self.pair_observations,
