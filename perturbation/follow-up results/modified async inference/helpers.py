...
## ADDED LINES (2026/08/31)
def resize_robot_observation_image(image: torch.tensor, resize_dims: tuple[int, int, int]) -> torch.tensor:
    assert image.ndim == 3, f"Image must be (C, H, W)! Received {image.shape}"
    image = image.permute(2, 0, 1)
    # CHANGED (2026/08/31, v2): pass the raw full-res frame through, converting
    # uint8->[0,1] only. v1 pad-resized to 256x256 here (aspect fix over the
    # original squash), but that still double-resizes: SmolVLA pad-resizes to
    # 512x512 internally (modeling_smolvla.py), and sync rollouts feed it raw
    # 640x480 — proven geometry. Match sync exactly: no resize here at all.
...
#     dims = (resize_dims[1], resize_dims[2])
#     image_batched = image.unsqueeze(0)
#     # CHANGED (2026/08/31): plain F.interpolate squashed 640x480 -> 256x256
#     # (aspect destroyed, ~1.33x horizontal compression), while SmolVLA
#     # trains/deploys through aspect-preserving resize_with_pad (left/top pad,
#     # pad_value=0 — see modeling_smolvla.py prepare_images). The squash caused
#     # a systematic lateral grasp offset in remote inference. Match training
#     # semantics. Float [0,1] conversion done here (was previously downstream
#     # in prepare_image, whose uint8 check our float output now bypasses).
