...
from .remote import RemoteSyncInferenceEngine # ADDED 2026/09/01
...
def create_inference_engine(
    ...
    *,
    ...
    if isinstance(config, RemoteInferenceConfig):
        return RemoteSyncInferenceEngine(
            server_address=config.server_address,
            policy_type=config.policy_type,
            policy_path_on_server=config.policy_path_on_server,
            policy_device=config.policy_device,
            actions_per_chunk=config.actions_per_chunk,
            image_rename=config.image_rename,
