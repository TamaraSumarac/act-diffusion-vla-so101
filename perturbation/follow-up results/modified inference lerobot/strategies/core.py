...
class RolloutStrategy(abc.ABC):
    ...
    def _return_to_initial_position(hw: HardwareContext, duration_s: float = 3.0, fps: int = 50) -> None:
        ...
        try:
            current_obs = robot.get_observation()
            current_pos = {k: v for k, v in current_obs.items() if k in target}
            # ADDED (2026/09/02): homing witness — during the diffusion remote pilot
            # eval, ~7/20 episodes started from the previous episode's end pose.
            # Prints both key namespaces to catch a silent empty-intersection no-op.
            logger.info(
                "Homing: %d/%d keys matched | obs keys: %s | target keys: %s",
                len(current_pos), len(target or {}),
                sorted(current_obs.keys())[:8], sorted((target or {}).keys()),
            )
            steps = max(int(duration_s * fps), 1)
            for step in range(1, steps + 1):
                t = step / steps
            ...
            # ADDED (2026/09/02): settle phase. With max_relative_target set,
            # every command is re-anchored to the arm's ACTUAL position, so when
            # the fixed-duration loop expires the last command stops short of
            # home (unclamped runs were rescued by the motor driving to its last
            # goal position on its own). Re-assert the true target until
            # converged or timeout; diagnosed 2026/09/02 via clamp-warning spam
            # showing safe_goal_pos falling ~13 deg behind the interpolation
            # clock over a 1 s homing window.
            err = float("inf")
            settle_deadline = time.perf_counter() + 5.0
            while time.perf_counter() < settle_deadline:
                obs = robot.get_observation()
                err = max(abs(obs[k] - target[k]) for k in target if k in obs)
                if err < 2.0:
                    break
                robot.send_action(dict(target))
                precise_sleep(1 / fps)
            if err >= 2.0:
                logger.warning("Homing settle timed out; residual error %.1f", err)
        except Exception as e:
            logger.warning("Could not return to initial position: %s", e)
